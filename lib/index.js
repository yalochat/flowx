  ...
  validateTransition (instance, action) {
    return new Promise((resolve, reject) => {
      if (instance.currentState.transitions) {
        Promise.reduce(instance.currentState.transitions, (result, transition) => {
          console.log("Check last result: %j", result)
          if(result.match) {
            return result
          }

          return Matchers
            .match(transition, action, { cache: result.cache })
            .then(match => {
              if(match.match) {
                return match
              } 
        ...
        .then((transition_match) => {
          console.log(`Transition found: ${action}`)
          return resolve(transition_match.rule)
        })
        .catch((error) => {
