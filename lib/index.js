...
class Flow {
  ...
  searchNextState(stateName, global) {
    return new Promise((resolve, reject) => {
      const newState = this.model.states.find(state => (
  ...
  validateTransition(instance, action) {
    ...
    return Promise.reduce(instance.currentState.transitions, (result, transition) => {
      ...
      .then((ruleMatch) => {
        const rule = ruleMatch.match ? ruleMatch.rule : (ruleMatch.action.value || ruleMatch.action)
        return rule
      })
      .catch((error) => {
  ...
  getState(instance, data) {
    ...
    return new Promise((resolve, reject) => {
      if (data && data.action) {
        return this.validateTransition(updatedInstance, data.action)
          .then((transition) => {
            console.log('Moving to transition: %j', transition)
            return Promise.props({
              transition,
              nextState: this.searchNextState(
          ...
          .then(({ transition, nextState }) => {
            if (transition.use) {
              updatedInstance.middlewares[transition.use](updatedInstance.currentState, () => {
                  ...
                  if (updatedInstance.currentState.onEnter) {
                    if (updatedInstance.currentState.onEnter.emit) {
                      this
                        .internalEmitter
                        .emit(
                          updatedInstance.currentState.onEnter.emit,
                          updatedInstance.currentState.onEnter.data,
                        )
                    }
                  }
            ...
            return this.goToDefault(updatedInstance)
              .then(state => resolve({ state, transition: {} }))
  ...
  goToDefault(instance) {
    return new Promise((resolve, reject) => {
      this.searchNextState('default', false).then((nextState) => {
        instance.currentState = nextState // eslint-disable-line
        client.set(instance.id, instance, this.model.ttl, (err) => {
          if (err) {
            return reject(err)
          }
          if (_.get(instance, 'currentState.onEnter.emit', false)) {
            this.internalEmitter
              .emit(instance.currentState.onEnter.emit, instance.currentState.onEnter.data)
          }
          return resolve(instance.currentState)
        })
      }).catch(err => resolve({ template: err }))
    })
  }
